...
/**
* This struct is use to store name of the specified interface
...
BPF_ARRAY(name_map, struct name_buf, 1);
...
TRACEPOINT_PROBE(net, net_dev_start_xmit){
    ...
    iph = skb_to_iphdr(skb);
    if (iph->protocol != IPPROTO_TCP) {
        return 0;
    }
    tcph = skb_to_tcphdr(skb);
    if (tcph->source != htons(9007)) {
        return 0;
    }
